...
const PendingCommentItem = ({ blog }) => {
  const { refresh, setRefresh } = useContext(CreateContext);

  const comment = blog?.map((comment) => comment);

  const handleAccept = (id) => {
    fetch(`https://api.introbangla.com/api/v1/blog/comment/${id}`, {
      method: "PATCH",
      headers: {
            ...
            <div className="border  flex">
              ...
              <div className="flex justify-center items-center w-[40%] border-r">
                <div className="text-center text-sm">
                  ...
                  <h1 className="mt-2 ">
                    <span className="font-bold">Comment :</span> {item?.content}
                  </h1>
                </div>
              </div>
                ...
                <div>
                  <div>
                    <button
                      ...
                      className="btn btn-xs btn-primary font-bold text-white"
                    >
                      Accept
                    </button>
                  </div>
                  <div>
                    <button
                      ...
                      className="btn btn-xs btn-error mt-2 font-bold text-white"
                    >
                      Delete
                    </button>
                  </div>
