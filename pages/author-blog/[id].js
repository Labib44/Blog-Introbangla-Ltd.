...
const AuthorBlog = () => {
    ...
    const authorBlogs = blogs?.filter(
        (authorBlog) => authorBlog.authorName === id
    );
    const filterAuthorBlogs = authorBlogs?.filter((authorBlog) => {
        if (searchValue === "") {
            return authorBlog;
        }
        return authorBlog.title
            .toLowerCase()
    ...
    if (filterAuthorBlogs?.length) {
        content = filterAuthorBlogs.map((filterAuthorBlog) => (
            <CategoryBlogItem
                key={filterAuthorBlog._id}
                blog={filterAuthorBlog}
            />
        ));
    }

    if (authorBlogs.length && !searchValue) {
        content = authorBlogs.map((authorBlog) => (
            <CategoryBlogItem key={authorBlog._id} blog={authorBlog} />
        ));
    }

    if (!filterAuthorBlogs?.length) {
        content = (
            <h1 className="text-center text-2xl font-bold text-error min-h-[30vh] pt-10">
                    ...
                    <div className="sm:flex justify-between items-center mb-5">
                        ...
                        <div class="mb-3 md:w-[40%] w-full">
                            <div class="input-group relative flex  items-stretch w-full mb-4">
                                ...
                                <button
                                    class="btn px-6 py-2.5 bg-warning text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-orange-500 hover:shadow-lg focus:bg-orange-500  focus:shadow-lg focus:outline-none focus:ring-0 active:bg-orange-800 active:shadow-lg transition duration-150 ease-in-out flex items-center"
                                    type="button"
                                    id="button-addon2"
                                >
                                    <svg
